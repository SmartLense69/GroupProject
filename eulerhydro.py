...
def phigrad(r, n, rho0, phi0):
    out = ((-((1/n)-1)/(rho0))*phi0**2)-((2/r)*phi0)-((rho0**(2/(1/n)))*consts.G*4*np.pi / K*(1+(1/n)))
    return out
...
def euler1(h, n, func1, ic1, func2=None, ic2=None):
    ...
    for i in range(1, steps):
        if func2 is not None:
            phisol[i] = phisol[i-1] + h*func1(rvalues[i-1], n, rhosol[i-1], phisol[i-1])
        rhosol[i] = rhosol[i-1] + h*func2(phisol[i-1])

    return rvalues, rhosol


result = euler1(100, 1, phigrad, 0, rhograd, 1e9)
